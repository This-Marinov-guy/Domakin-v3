...
const OUTPUT_FILE = path.join(__dirname, '../public', config.sitemap.outputFile);
...
function loadStaticProperties() {
  try {
    const translationsPath = path.join(__dirname, '../locales/en/translations.json');
    const translations = JSON.parse(fs.readFileSync(translationsPath, 'utf8'));
    
    if (translations.FOR_RENT && Array.isArray(translations.FOR_RENT)) {
      console.log(`📋 Found ${translations.FOR_RENT.length} static properties in translations`);
      return translations.FOR_RENT.filter(property => property.id && !property.hidden);
    }
    
    console.log('⚠️ No FOR_RENT properties found in translations');
    return [];
  } catch (error) {
    console.error('❌ Error loading static properties from translations:', error.message);
    return [];
  }
}
...
async function fetchBlogPosts() {
  try {
    ...
    console.log(`🔗 Endpoint: ${API_CONFIG.baseUrl}${API_CONFIG.endpoints.blogPosts}`);
    
    const response = await axios.get(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.blogPosts}`, {
      headers: {
    ...
    if (response.data?.status && Array.isArray(response.data.data)) {
      ...
      console.log(`✅ Found ${response.data.data.length} blog posts (${postsWithSlugs.length} with slugs)`);
...
async function fetchProperties() {
  try {
    ...
    console.log(`🔗 Endpoint: ${API_CONFIG.baseUrl}${API_CONFIG.endpoints.properties}`);
    
    const response = await axios.get(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.properties}`, {
      headers: {
        'Accept-Language': API_CONFIG.language
      },
      timeout: API_CONFIG.timeout
    ...
    if (response.data?.status && Array.isArray(response.data.data)) {
      const visibleProperties = response.data.data.filter(property => property.id);
      console.log(`✅ Found ${response.data.data.length} properties (${visibleProperties.length} visible)`);
...
function generateSitemap(blogPosts = [], properties = [], staticProperties = []) {
...
`;
  ...
  // Add blog posts (only real data from API)
...
async function main() {
  try {
    ...
    const totalUrls = staticPages.length + blogPosts.length + properties.length + staticProperties.length;
