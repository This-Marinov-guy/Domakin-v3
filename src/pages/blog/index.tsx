...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    // Only add test post if no real posts were found or if in development mode
    let allPosts = blogPosts;
    
    if (!Array.isArray(blogPosts) || blogPosts.length === 0 || process.env.NODE_ENV === 'development') {
      // Only add test post if no real data or in development
      console.log('[Blog Index] Adding test post as fallback or for development');
      const testPost = {
        id: "test-1",
      ...
      // If blogPosts exists and has items, add test post at the end, otherwise just use test post
      allPosts = Array.isArray(blogPosts) && blogPosts.length > 0 ? 
        (process.env.NODE_ENV === 'development' ? [...blogPosts, testPost] : blogPosts) : 
        [testPost];
    }
    
    // Log final post count
    console.log(`[Blog Index] Total posts being rendered: ${allPosts.length}`);
    if (allPosts.length > 0) {
      console.log(`[Blog Index] First rendered post title: ${allPosts[0].title || 'No title'}`);
    }
